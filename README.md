# TORPPA-matrix
Development of a Pipeline for Putative Functional Homolog identification. This repository stores data and scripts used for the generation and assessment of TORPPA matrices in comparison to BLOSUM matrices. Additionally, data and scripts used for the TM-Vec/GASS approach are available.

Substitution matrix based on SeqPredNN (https://github.com/falategan/SeqPredNN)

BLAST+ source files used in this project (2.14.1) are available here: https://stellenbosch-my.sharepoint.com/:f:/g/personal/22042636_sun_ac_za/ElZKYJQcacBGib6HmJdSe4UBgASEkgZaO8vmaFO869cJoA?e=VOptg4

BLAST+ pdbaa searches were performed using the preformatted database available here: https://ftp.ncbi.nlm.nih.gov/blast/db/pdbaa.tar.gz

TM-Vec code used in this study is available here: https://github.com/tymor22/tm-vec?tab=readme-ov-file

GASS-Metal code used in this study is available here: https://github.com/sandroizidoro/gassmetal-local
. GASS-Metal can also be used via their web server: https://gassmetal.unifei.edu.br/

Any further information can be acquired by emailing me at jetorppa@gmail.com
